...
def predict_proba(X, trained_params):
    X = jnp.asarray(X, dtype=jnp.float32)

    if X.ndim == 1:
        raw = circuit(trained_params, X)
        return onp.asarray(raw_to_probability(raw))
...
def evaluate_at_threshold(y_true, probs, threshold):
    y_true = onp.asarray(y_true).reshape(-1).astype(int)
    probs = onp.asarray(probs).reshape(-1)
...
def plot_threshold_sweep(df_metrics, save_path=None):
    plt.figure(figsize=(8, 4))
...
if __name__ == "__main__":
    ...
    if "train_losses" in globals() and len(train_losses) > 0:
        plot_training_loss(train_losses, save_path=os.path.join(out_dir, "training_loss.png"))

    # Base evaluation at default threshold
    default_threshold = 0.15
    probs_test = predict_proba(X_test, trained_params)
    base_eval = evaluate_at_threshold(y_test, probs_test, default_threshold)

    y_pred = base_eval["y_pred"]
    y_true = onp.asarray(y_test).reshape(-1)

    print("\nTest inference results (first 10 samples):")
    preview_n = min(10, len(y_pred))
    ...
    print("\n" + "=" * 60)
    print("FINAL RESULTS @ threshold = " + str(default_threshold))
    print("=" * 60)
    print(f"Accuracy : {base_eval['accuracy']:.4f}")
    print(f"Precision: {base_eval['precision']:.4f}")
    print(f"Recall   : {base_eval['recall']:.4f}")
    print(f"F1 Score : {base_eval['f1']:.4f}")
    print("Confusion Matrix [ [TN, FP], [FN, TP] ]:")
    print(base_eval["confusion_matrix"])
    print(f"TN={base_eval['tn']} FP={base_eval['fp']} FN={base_eval['fn']} TP={base_eval['tp']}")

    plot_confusion_matrix(
        base_eval["confusion_matrix"],
        default_threshold,
        save_path=os.path.join(out_dir, f"confusion_matrix_t{default_threshold:.2f}.png"),
    )

    # Threshold sweep
    thresholds = onp.linspace(0.0, 1.0, 101)
    rows = []
    for t in thresholds:
        metrics = evaluate_at_threshold(y_test, probs_test, t)
        rows.append({
    ...
    print("  Best accuracy threshold:", float(df_metrics.loc[df_metrics["accuracy"].idxmax(), "threshold"]))
    print("  Best precision threshold:", float(df_metrics.loc[df_metrics["precision"].idxmax(), "threshold"]))
    print("  Best recall threshold:", float(df_metrics.loc[df_metrics["recall"].idxmax(), "threshold"]))
    print("  Best F1 threshold:", float(df_metrics.loc[df_metrics["f1"].idxmax(), "threshold"]))

    plot_threshold_sweep(
        df_metrics,
        save_path=os.path.join(out_dir, "threshold_sweep.png"),
    )

    best_f1_idx = df_metrics["f1"].idxmax()
    best_row = df_metrics.loc[best_f1_idx]
    best_threshold = float(best_row["threshold"])
    best_eval = evaluate_at_threshold(y_test, probs_test, best_threshold)

    print("\n" + "=" * 60)
    print("BEST F1 RESULTS @ threshold = " + str(round(best_threshold, 4)))
    print("=" * 60)
    ...
    print(f"TN={best_eval['tn']} FP={best_eval['fp']} FN={best_eval['fn']} TP={best_eval['tp']}")

    plot_confusion_matrix(
        best_eval["confusion_matrix"],
        best_threshold,
        save_path=os.path.join(out_dir, f"confusion_matrix_best_f1_t{best_threshold:.2f}.png"),
    )
